...
const Assessments: React.FC = () => {
    ...
    // ── CHANGE 1: Toggle Draft ↔ Published ───────────────────────────────────
    ...
    // ── CHANGE 2: Open view links modal ──────────────────────────────────────
    ...
    // ── CHANGE 3: Navigate to CreateLink — title passed for dynamic heading ──
            ...
            <div className="table-card">
                {loading ? (
                    ...
                    <div className="empty-state">
                        ...
                        {!search && <button className="btn btn-primary btn-sm" style={{ marginTop: '16px' }} onClick={() => navigate('/assessments/create')}>+ Create Assessment</button>}
                    </div>
                ) : (
                    <table className="assessments-table">
                        <thead>
                            <tr>
                                <th>Assessment</th><th>Questions</th><th>Duration</th>
                                <th>Marks</th><th>Negative</th><th>Status</th><th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filtered.map((a, i) => {
                                ...
                                return (
                                    <tr key={id || i} style={{ animationDelay: `${i * 0.03}s` }}>
                                        ...
                                        <td><span className="marks-val">{qs * mpq}</span><span className="marks-sub"> ({mpq}/Q)</span></td>
                                        <td>
                                            {neg > 0
                                        ...
                                        {/* CHANGE 1 — clickable status toggle */}
                                        <td>
                                            <button
                                        ...
                                        {/* CHANGE 2 — 🔗 Create Link  |  👁 View Links */}
                                        <td>
                                            <div className="action-btns">
                                                <button className="act-btn act-link" title="Create Exam Link" onClick={() => handleCreateLink(a)}>🔗</button>
                                                <button className="act-btn act-view" title="View Links"       onClick={() => handleViewLinks(a)}>👁</button>
                                            </div>
                                        </td>
                                    </tr>
                                );
                ...
                <div className="modal-overlay open" onClick={e => { if (e.target === e.currentTarget) setQrModal(p => ({ ...p, open: false })); }}>
                    <div className="modal" style={{ maxWidth: '360px', width: '90vw' }}>
                        ...
                        <div className="modal-body" style={{ textAlign: 'center', padding: '28px 24px' }}>
                            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(qrModal.url)}`}
                                alt="QR" style={{ width: 200, height: 200, borderRadius: 12, border: '1px solid var(--border)' }} />
